...
from inspect_charxiv.utils import convert_descriptive_question, convert_reasoning_question
...
def load_charxiv_dataset(subset: Literal["descriptive", "reasoning"] | None = None, category: FieldOfStudy | list[FieldOfStudy] | None = None, correct_targets: bool = True) -> Dataset:
    ...
    dataset = hf_dataset(
        ...
        split="validation",
        sample_fields=_make_record_to_sample(subset=subset, correct_targets=correct_targets),
        revision="f441eb632fc62f6f777830a0f47619e6e86459b0", # The latest commit to the CharXiv repo as of Aug 11, 2026.
    )
    if category is not None:
        dataset = dataset.filter(lambda sample: sample.metadata.get('field_of_study') in category)
    return dataset

# record_to_sample factory to retain task parameters for record_to_sample function getting passed to hf_dataset.
def _make_record_to_sample(subset: Literal["descriptive", "reasoning"] | None = None, correct_targets: bool = True) -> RecordToSample:
    ...
    def record_to_sample(record: Dict[str, str | int | None]) -> Sample | List[Sample]:
        if (subset == None):
            return [convert_descriptive_question(question_index=1, input_sample=record, correct_targets=correct_targets), 
                convert_descriptive_question(question_index=2, input_sample=record, correct_targets=correct_targets), 
                convert_descriptive_question(question_index=3, input_sample=record, correct_targets=correct_targets), 
                convert_descriptive_question(question_index=4, input_sample=record, correct_targets=correct_targets), 
                convert_reasoning_question(input_sample=record, correct_targets=correct_targets)]
            ...
            return [convert_reasoning_question(input_sample=record, correct_targets=correct_targets)]
        else:
            raise ValueError(f"Invalid subset value. Must be 'descriptive', or 'reasoning'. Received: {subset}")
